...
public class SwrtjLabelProvider extends DefaultEObjectLabelProvider {
	...
	String image(FieldName f) {
		...
		if(field instanceof FieldDeclaration)
			return "field_public_obj.gif";
		else if(field instanceof RequiredField)
	...
	String text(Type type) {
		return (type.getInterfaceType() != null ? type.getInterfaceType().getName()
				: type.getPrimitiveType());
	}

	String image(MethodName m) {
		Method method = lookup.getOwnerMethod(m);
		if(method instanceof ProvidedMethod)
			return "methdef_obj.gif";
		else if(method instanceof RequiredMethod)
	...
	String text(MethodName m) {
		return m.getName() + "(" + listToText((EList<Parameter>)lookup.getParameterList(m)) + ") : "
				+ text(lookup.getReturnType(m));
	}
	...
	/*String image(Expression e) {
		if (e.eContainer().eClass().getClassifierID() == SwrtjPackage.FILE) {
